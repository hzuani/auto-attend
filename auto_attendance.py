...
def process_attendance(excel_file):
    ...
    try:
        ...
        # ★ [수정됨] 셀 병합 해결 마법! (빈칸을 같은 학생 데이터로 채워줌)
        df['번호'] = df['번호'].ffill()
        df['성명'] = df['성명'].ffill()
    ...
    for _, row in df.iterrows():
        if pd.isna(row['출결구분']): continue
        s_no = str(int(row['번호'])) if not pd.isna(row['번호']) else "?"
        s_name = row['성명']
        key = f"{s_no}_{s_name}"
        if key not in students: students[key] = []
        students[key].append({
            'date': extract_date(row['일자']),
            'type': row['출결구분'], 
            'reason': row['사유'] if not pd.isna(row['사유']) else "",
    ...
    for key, records in students.items():
        ...
        for r in normal_absences:
            k = r['type'] 
            if k not in grouped_dict: grouped_dict[k] = []
        ...
        for _, r_list in grouped_dict.items():
            ...
            for i in range(1, len(r_list)):
                if (r_list[i]['date'] - r_list[i-1]['date']).days <= 7:
                    cur.append(r_list[i])
                else:
            ...
            for group in cur_groups:
                ...
                fname = os.path.join(OUTPUT_DIR, f"{s_no}번_{s_name}_결석_{s_dt.strftime('%m%d')}.hwp")
                hwp.SaveAs(fname) 
                
                pdf_name = fname.replace(".hwp", ".pdf")
        ...
        for r in rec_absences:
            k = r['type'] 
            if k not in grouped_rec_abs: grouped_rec_abs[k] = []
        ...
        for _, r_list in grouped_rec_abs.items():
            ...
            for group in cur_groups:
                ...
                fname = os.path.join(OUTPUT_DIR, f"{s_no}번_{s_name}_인정결석_{s_dt.strftime('%m%d')}.hwp")
                hwp.SaveAs(fname)
        ...
        for item in rec_others:
            ...
            fname = os.path.join(OUTPUT_DIR, f"{s_no}번_{s_name}_{item['type']}_{d_dt.strftime('%m%d')}.hwp")
            hwp.SaveAs(fname)
    ...
    return f"작업 완료!\n총 {count_files}건의 서류가 병합되어\n1개의 [통합본 PDF]가 생성되었습니다."
